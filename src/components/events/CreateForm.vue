<template>
  <Dialog :focus-outside="false">
    ...
    <DialogContent class="sm:max-w-2xl max-h-[90vh] overflow-y-auto">
      <DialogHeader>
        <DialogTitle>Select sport type</DialogTitle>
        <DialogDescription />
      </DialogHeader>
      ...
      <div class="flex items-center space-x-2">
        <Input placeholder="Other organizers" />
        <Button variant="outline" size="icon">
          <Plus class="w-4 h-4" />
        </Button>
      </div>
      <div class="grid gap-4 mb-4">
        <Input placeholder="Event tags" class="col-span-2" />
...
import { ArrowRight, Plus, Upload, X } from 'lucide-vue-next';
import { ref } from 'vue';

// Replace icons with appropriate lucide ones or custom SVGs if necessary
// const Icon = defineComponent({
//   props: ['name'],
//   setup(props) {
//     return () => h('div', { class: 'w-5 h-5 bg-gray-300 rounded-full' }); // placeholder icon
//   },
// });
const sports = [
